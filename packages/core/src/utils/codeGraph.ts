export function buildCodeDependencyGraph(
  ...
  for (const conn of connections) {
    ...
    if (out.length + conn.length + 60 > maxChars) {
      out +=
        "  // ... [Graph truncated to save context. Core architecture shown above.]\n";
      break;
    }
    out += `${conn}\n`;
  }
