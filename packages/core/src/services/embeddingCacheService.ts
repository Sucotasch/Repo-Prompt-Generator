...
export class EmbeddingCacheService {
   ...
   */
  public static async getEmbedding(
    text: string,
    model: string,
    repoUrl: string,
  ): Promise<number[] | null> {
   ...
   */
  public static async saveEmbedding(
    text: string,
    model: string,
    embedding: number[],
    repoUrl: string,
   ...
   */
  public static async pruneUnusedCaches(
    ...
    try {
      ...
      const activity: Record<string, number> = activityStr
        ? JSON.parse(activityStr)
        : {};
      ...
      for (const name of namespacesToDelete) {
        console.log(
          `[EmbeddingCache] Pruning stale repo cache (inactive > 5m): ${name}`,
        );
    ...
    try {
      ...
      for (const name of cacheNames) {
        if (
          ...
          name.includes("repo-prompt-embeddings")
        ) {
          await caches.delete(name);
        }
      }
