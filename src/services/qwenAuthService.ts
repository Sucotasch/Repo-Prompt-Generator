...
export async function startDeviceAuth() {
  ...
  try {
    const data: any = await invoke('qwen_device_code', {
...
export async function pollDeviceToken(deviceCode: string, codeVerifier: string) {
  const { invoke } = await import('@tauri-apps/api/core');
  try {
    const data: any = await invoke('qwen_poll_token', {
      clientId: QWEN_OAUTH_CLIENT_ID,
      deviceCode: deviceCode,
      codeVerifier: codeVerifier
    });

    if (data.http_status === 400 && data.error === 'authorization_pending') {
      return { status: 'pending' };
    }
    if (data.http_status === 429 && data.error === 'slow_down') {
