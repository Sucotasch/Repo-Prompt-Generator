...
export async function selectLocalFolderWithTauri(maxFiles: number, existingPath?: string): Promise<RepoData> {
  ...
  const sourceFiles: { path: string, content: string }[] = await invokeTauri('scan_local_repository', { path: selected });
  ...
  const getFileScore = (filePath: string): number => {
    ...
    const coreDirs = ['src/', 'lib/', 'app/', 'core/', 'pkg/', 'internal/'];
    if (coreDirs.some(d => lowerPath.startsWith(d) || lowerPath.includes('/' + d) || lowerPath.includes('\\' + d))) {
      score += 20;
    }
