...
export async function processLocalFolder(files: FileList, maxFiles: number): Promise<RepoData> {
  ...
  let readme = '';
  const readmeFile = filteredFiles.find(f => (f.webkitRelativePath || f.name).toLowerCase().endsWith('readme.md'));
  if (readmeFile) {
  ...
  for (const depName of depFiles) {
    const depFile = filteredFiles.find(f => (f.webkitRelativePath || f.name).endsWith(depName));
    if (depFile) {
      dependencies += `\n--- ${depName} ---\n${await depFile.text()}\n`;
    }
  }
  ...
  filesToFetch.sort((a, b) => getFileScore(b.webkitRelativePath || b.name) - getFileScore(a.webkitRelativePath || a.name));

  const limit = Math.min(Math.max(1, Number(maxFiles) || 5), 200);
  filesToFetch = filesToFetch.slice(0, limit);

  const finalSourceFiles = await Promise.all(filesToFetch.map(async f => ({
  ...
  const repoName = fileArray.length > 0 ? (fileArray[0].webkitRelativePath.split('/')[0] || 'local-folder') : 'local-folder';

  return {
    info: { owner: 'local', repo: repoName, defaultBranch: 'local', branch: 'local', description: 'Local folder upload' },
    tree: finalTreePaths,
    readme,
    dependencies,
